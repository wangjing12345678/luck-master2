<template>
  <div>
    <div class="circle">
      <div class="circle-in" ref="rotate-content" :id="elId" :class="currState" @transitionend="_transEnd" @webkitTransitionEnd="_transEnd">
  ...
  methods: {
    ...
    setDeg() {
      ...
      this.waitTime = setInterval(() => {
        // 避免旋转时间过短 角度小于9000时候在停止
        if (this.waitDeg < 9000) {
          this.timeOut = false;
        } else {
          this.timeOut = true;
        }
        tempNum += 1;
        if (this.waitState && this.timeOut) {
          this.calcRandomDeg();
          // 计算角度360倍数 加上奖项所需转动角度
          this.flag = 360 - (this.waitDeg % 360) + this.waitDeg;
          ...
          this.waitDeg = this.flag + -playDeg;
        } else {
          this.waitDeg = 20 * tempNum;
        }
        this.el.style.cssText = `-webkit-transform:rotate(${this.waitDeg}deg);
    ...
    reset() {
      ...
      this.timeOut = false;
      this.el.style.cssText = `-webkit-transform:rotate(${this.deg}deg);
